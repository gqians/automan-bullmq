...
const BULL_REDIS_URI =
  process.env.BULL_REDIS_URI || 'redis://:123456@127.0.0.1:16379';

const redisInstances = new Map<string, Redis.Redis>();

export function createBullConnection(
  type: 'queue' | 'worker' | 'scheduler' | 'events' | 'custom',
) {
  ...
  if (!client) {
    throw new Error(
      `No redisio connection provided to BULL (${BULL_REDIS_URI})!`,
    );
  }
...
export function connectRedis(uri: string, opts?: RedisOptions): Redis.Redis {
  ...
  if (cs.scheme === 'redis' || cs.scheme === 'rediss') {
    ...
    throw new Error(
      `Unsupported connection string provided to connectRedis() method: ${uri}`,
    );
  }
